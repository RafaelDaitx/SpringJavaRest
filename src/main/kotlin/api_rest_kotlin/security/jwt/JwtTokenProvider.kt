...
class JwtTokenProvider {
    ...
    fun refreshToken(refreshToken: String): TokenVO {
       ...
       private fun getAccessToken(username: String, roles: List<String?>, now: Date, validity: Date): String {
        val issuerURL: String = ServletUriComponentsBuilder.fromCurrentContextPath().build().toUriString() // pega o endereco da api
        return JWT.create()
    ...
    private fun getRefreshToken(username: String, roles: List<String?>, now: Date): String? {
        val validityRefreshToken = Date(now.time + validityInMilliseconds * 3)//multiplica o tempo por 3
    ...
    private fun decodedToken(token: String): DecodedJWT {
        ...
        return verifier.verify(token) //descriptografa a shave hmac257 (token)
    }

    fun resolveToken(req: HttpServletRequest): String? {
        val bearerToken = req.getHeader("Authorization")
        //Bearer jfsakjgflaslghla09213i9j201i

        return if (!bearerToken.isNullOrBlank() && bearerToken.startsWith("Bearer ")){
            bearerToken.substring("Bearer ".length) //remove o Bearer do token
        } else null
    }
